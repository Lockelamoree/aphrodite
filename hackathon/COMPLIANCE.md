...
| 4 | **Screenshots** (a required field on the submission form) | **PASS** — ten stills captured 2026-08-12 from a production build with the access gate configured, committed at `submission/screenshots/` with a note on what each one proves. The stale 2026-07-26 PNGs are superseded | upload them with the entry |
| 5 | Demo video, **1–3 minutes**, showing the project functioning on the intended device and **explaining which YouCam API is used** | **PASS on the artifact** — recut 2026-08-15: `submission/aphrodite-demo-2026-08-15-voiced.mp4`, **163.6 s (2:44)**, 1920×1080 @30, inside the window with 16 s of margin, from a production build. Five acts; every caption anchored to the panel it describes and each YouCam API named as it appears. **Spoken** narration (OpenAI `gpt-4o-mini-tts`, voice `marin`) with burned captions in the same words, audio normalised to −14.4 LUFS; the spoken track was transcribed back and checked against the vocabulary law. The judge access code is never on camera. All earlier cuts, including the 2026-08-12 voiced one, are do-not-publish | nothing — uploaded https://youtu.be/ucrsgkAdgyE on 2026-08-16, unlisted, verified playable from outside with no cookies |
...
| 4 — public repo + MIT + README chapter + video under 180 s | **PASS** — https://youtu.be/ucrsgkAdgyE, 164 s, unlisted; the repo is public under MIT and the README carries the chapter |
