...
| The agentic (LLM) engine running for a judge | **Degraded as of 2026-08-15, and it used to say the opposite.** This row read "`/healthz` reports `agentic_engine: live` on gpt-5"; every probe on 2026-08-14 and 2026-08-15 returned `key_present_unverified` with `agentic_engine_reason: "This operation was aborted"` and `degraded: true`. The committed receipts from 2026-08-12 embed `agentic_engine: "live"`, so it worked then and the free `GET /v1/models` probe now times out. Treat the second engine as an architecture note, never a lead claim, until `/healthz` says `live` again. | devpost, README |
