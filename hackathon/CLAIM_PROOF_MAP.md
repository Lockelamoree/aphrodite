...
| Demo video length | **163.6 s (2:44)**, 1920×1080 @30, h264 4.85 Mbps + aac 48 kHz | `ffprobe` on `submission/aphrodite-demo-2026-08-15-voiced.mp4` | 2026-08-15 | the submission, once uploaded. Inside the published 1–3 min window, 16 s of margin |
| Demo video, hosted | **https://youtu.be/ucrsgkAdgyE** — unlisted, 164 s | fetched `youtube.com/watch?v=…` from a shell with no cookies: `playabilityStatus` OK, `lengthSeconds` 164, page marked UNLISTED | 2026-08-16 | Devpost video field, README, `config.json` |
| Devpost entry | **submitted** 2026-08-16, editable until the deadline | Devpost said "Project submitted!" and the public page devpost.com/software/aphrodite-xplekr embeds the video and shows 11 captioned stills | 2026-08-16 | `config.json` submission.state |
| Demo video narration | **spoken**, OpenAI `gpt-4o-mini-tts` voice `marin`, captions carrying the same words, audio at **−14.4 LUFS** | `scripts/build-demo-video.mjs` synthesises each line in `submission/narration.txt` and holds its footage to that length; the caption is drawn from the same string. Loudness measured with `ffmpeg -af loudnorm=print_format=summary`. NOTE: with this provider the narration text leaves the machine — `TTS_PROVIDER=piper` is the offline path | 2026-08-15 | the reel |
| Demo video vocabulary | **no banned word in the spoken track** | the delivered audio was transcribed back with `faster-whisper` small.en and grepped for `camouflage\|flawless\|glowing\|perfect skin\|flaws\|simulated\|glow` — zero hits. This is the one artifact a source grep structurally cannot audit | 2026-08-15 | the reel |
...
| Live runs claimed via the app | **2 of 8** as of 2026-08-16 | `/healthz` `live_runs_used`; both are operator test runs against the deployed instance with the judge cookie, confirmed by the operator. The public path cannot claim a run, so anonymous traffic cannot appear here | 2026-08-16 | `/healthz`, `config.json` youcam.unitsSpentLog |
