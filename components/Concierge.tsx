...
export function Concierge() {
  ...
  // Load the bundled sample selfie + full-body (as data URLs, so it behaves
  // exactly like a real upload in both fixture and live mode).
...
function Results({
  ...
  // Occasion-aware finish: for interviews, the relit portrait IS a
  // LinkedIn-ready headshot (same verified Photo-Lighting endpoint, honest framing).
  const isInterview = /interview/i.test(state.board?.headline ?? occasion ?? "");
  const finishTitle = isInterview ? "Interview headshot" : "Occasion lighting";
  const finishCaption = isInterview
    ? "YouCam AI Photo Lighting · LinkedIn-ready"
    : "YouCam AI Photo Lighting · warm relight";
    ...
    <section className="aura-fade-up space-y-8">
      ...
      {board && (
        <div ref={boardRef} className="space-y-4">
          ...
          <LookBoardPanel board={board} />
          <RefineBar refine={refine} disabled={state.phase === "running"} />
        </div>
      )}
      ...
      <p className="aura-print-only mt-6 text-center text-xs text-muted">
        Generated by Aphrodite · every image is a real YouCam AI render · not a simulation
      </p>
    </section>
  );
}
  ...
  return (
    <div className="rounded-[var(--radius-card)] border border-line bg-surface p-5">
      ...
      <p className="mb-3 text-xs text-muted">
        0–100 · higher is healthier · lowest = where to focus · YouCam Skin Analysis
      </p>
...
function LookBoardPanel({ board }: { board: LookBoard }) {
        ...
        {board.shopping.length > 0 && (
          <div>
            ...
            <p className="mb-4 text-xs text-muted">
              Skincare + fashion in one basket · rendered on you first, so you buy with confidence.
            </p>
      ...
      <div className="space-y-3 border-t border-line pt-4">
        ...
        <details className="aura-no-print text-sm">
          ...
          <ul className="mt-2 space-y-1 text-muted">
            ...
            <li>Try-on rendered on the shopper → higher purchase confidence, fewer returns</li>
            <li>
