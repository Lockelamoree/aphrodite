...
Captured **2026-08-12** from a **production build** in demo mode (`YOUCAM_FIXTURES=1`,
...
3. This one, taken after the render fixtures were keyed to the person they depict.
...
| `05-countdown.jpg` | The look board | The hero is now the **try-on render of the person in the run**, captioned as the Apparel VTO API. The countdown differs in *kind* by how far off the date is |
| `06-priced-basket.jpg` | One cross-category priced basket | The retail case on screen rather than asserted |
| `07-provenance-ledger.jpg` | The provenance ledger | Reads "**3** Perfect Corp APIs produced these samples. Nothing was called just now." Three, not four: there is no captured relight of this face, so Photo Lighting is absent instead of borrowed |
